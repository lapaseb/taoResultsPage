...
function get_deliveries(){
	...
	try {
		...
		$deliveries = array();
		//Parcourt les sessions d'examens
...
function get_deliveries_resume(){
	...
	//Parcourt les examens pour reformer un tableau
	foreach($deliveries as $value){
		...
		//Variable qui indique si l'examen existe et donc si il doit être push dans le nouveau tableau ou alors être fusionné
		$pushArray = false;
		//Parcourt le nouveau tableau afin de détecter  si l'occurrence   du vieux tableau y est présente, si oui, il sera fusionné
		...
		//Si l'occurrence  du vieux tableau n'est pas présente dans le nouveau, alors on l'ajoute
		if(!$pushArray){
...
function get_results_by_delivery($delivery){
	...
	try {
		//Requête de sélection des résultats de l'examen passé en paramètre
		...
		$allResults = array();
		//Parcourt chaque résultats de l'examen
		foreach ($results_storage as $value) {
			//Requête qui sélectionne les résultats  de l'examen
			...
			//Parcourt tous les résultats (examen)
			foreach($results as $result){
				...
				//Parcourt toutes les questions d'un examen
				...
				array_push($Responses_array, array("label" => $itemLabel[0]['object'], "score" => $scoreToAdd, "totalScore" => $totalPointsToAdd, "responses" => $userResponses_array));
			}
...
function get_results_by_delivery_resume($deliveries){
	...
	//Parcourt tous les examens passés en paramètre
	foreach(explode(" ", $deliveries) as $id){
		//Parcourt tous les résultats de l'examen pour reformer le nouveau tableau
		foreach(get_results_by_delivery($id) as $result){
			$alreadyExist = false;
			//Parcourt le nouveau tableau pour vérifier que le test-taker de l'examen que l'on parcourt 
...
function getObtainableScoreOfItem($id){
	...
	if (file_exists($url)) {
		...
		//Accède à la balise responseDeclaration
		$responseDeclarations = $xmlDoc->getElementsByTagName( "responseDeclaration" );

		//Parcourt les balises de la balise "responseDeclaration" afin de trouver le score pouvant être obtenu sur la question
		foreach($responseDeclarations as $responseDeclaration){
			
			//Parcourt la balise mapEntry afin de calculer le score de la question
